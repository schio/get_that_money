...
class DataLoader():
    ...
    def get_candle_from_csv(self,path):
        cols = ("code", "date", "open", "close", "high", "low", "cr_close", "volume", "trading_value", "accumulate_sell_volume")
        ...
        if self.config.data.indicator.macd:
            for fastperiod in range(6, 30, 2):
                ...
                candle[f"macd_{fastperiod}_{slowperiod}_{signalperiod}"] = get_macd(candle, fastperiod, slowperiod, signalperiod)
        ...
        if self.config.data.indicator.macd:
            for fastperiod in range(5, 30, 5):
                for slowperiod in range(3, 30, 3):
                    candle[f"slow_stoch_k_{fastperiod}_{slowperiod}_{slowperiod}"], candle[f"slow_stoch_d_{fastperiod}_{slowperiod}_{slowperiod}"] = get_stoch(candle, fastperiod, slowperiod, slowperiod)
        
        if self.config.data.indicator.bollinger_bands:
            for timeperiod in range(3, 30, 2):
                candle[f"upperbb_{timeperiod}"], candle[f"middlebb_{timeperiod}"], candle[f"lowerbb_{timeperiod}"] = get_bbands(candle, timeperiod, int(timeperiod*0.4), int(timeperiod*0.4))
